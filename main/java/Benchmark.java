       ...
       "weather_sept_85.csv", "weather_sept_85.csv" };
            ...
            for (int i = 0; i < dataSources.length; i++) {
                ...
                // **********************************
                {
                    ...
                    File file_index = File.createTempFile(dataSet + seq, "idx");
                    file.deleteOnExit();
                    file_index.deleteOnExit();
                    final FileOutputStream fid = new FileOutputStream(file_index);
                    final DataOutputStream did = new DataOutputStream(fid);
                    final FileOutputStream fos = new FileOutputStream(file);
                    final DataOutputStream dos = new DataOutputStream(fos);
                    ArrayList<Long> offsets = new ArrayList<Long>();
                    ...
                    for (int k = 0; k < offsets.size() - 1 ; k++) {
                        ...
                        ImmutableConciseSet ic = ImmutableConciseSet.compact(ics);
                        for(int t = 0 ;t<=ic.getLastWordIndex();t++)
                        {
                        	did.writeInt(ic.get(t));
                        }
                        icss.add(ic);
                    }
                    did.close();
